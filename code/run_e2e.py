...
def st_predict_base(a):
    # base answers = run_qwen35_route_auto.sh (bf16, all qtypes) -> SUBMIT_Q35_PURE
    # -> SUBMIT_Q35_FULL_v2/subtask2_vqa.json, the layer every later wave sits on
...
def st_predict_8bit(a):
    # both Viterbi passes decoded 8-bit probs (wsd_pipeline.sh / wsd_v2_pipeline.sh
    # export no QUANT -> run_qwen35_vqa.py's "8bit" default); the answers are unused
    sharded_predict(a.work / "test_prep/vqa/processed/vqa_val.json",
